...
未配置密钥时接口会 fail-closed 返回 `503 INFINISYNAPSE_NOT_CONFIGURED`，原有规则匹配仍然可用。比赛演示录屏中的 AI 结果使用测试同款模拟 SSE，并在画面角标明确标注；只有配置真实 Key、成功返回真实 Task ID 且后台可查日志后，才应宣称真实平台调用。
